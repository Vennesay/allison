...
bot.on('ready', () => {
    ...
    bot.guilds.find(g => g.id == "427906722527707147").channels.find(c => c.name == "🎮game-bot-logs🎮").send(`Хэй! Я был запущен!`);
});

bot.on('message', async message => {
    if (message.channel.type == "dm") return
    if (message.author.bot) return
    if (message.content == "/pingbot") return message.reply("`я онлайн.`") && console.log(`Бот ответил ${message.member.displayName}, что он онлайн.`)
                                    ...
                                    message.channel.awaitMessages(response => response.member.id == message.member.id, {
                                        ...
                                        message.reply(`[CREATE GAME] => Укажите ID автора`).then(author_game => {
                                            message.channel.awaitMessages(response => response.member.id == message.member.id, {
                                                ...
                                                author = bot.guilds.find(g => g.id == "427906722527707147").members.find(m => m.id == collected_author.first().content);
                                                author_game.delete();
